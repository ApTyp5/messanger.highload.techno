...
|Balancers L7| 3 (+ 3 доп. для отказоустойчивости) | 6 |
|DNS Balancres| 1 (+ 2 доп. для отказоуйстойчивости) | 3 |
...
2. На случай падения балансировщика есть 1 запасной, который соединён в redunancy group с ним по протоколу CARP. 
Если и балансировщик выйдет из строя, то это повлечет за собой недоступность 1-го из virtual ip, 
что обнаружится при heart-beat-проверке (которая расположена на DNS-серверах), что будет причиной
удаления этого virtual ip из адресных записей DNS-таблиц - сервис останется доступным.
