...
Расчет памяти на одно сообщение
------------------------
* В [Исследовании](https://crushhapp.com/blog/k-wrap-it-up-mom) приведено количество символов,
которые люди отправляют в одном сообщении.
* Пусть средний возраст пользователя мессенджера будет равен 25 годам. Следовательно средняя длина сообщений равна 35-ти символам (70 байт максимум для UTF-8 русского языка).
* Также вместе с сообщением передается id пользователя (5 символов в среднем),
* id отправителя (5 символов в среднем)
* и время отправки сообщения (10 символов максимум).
 
 В итоге получается, что одно среднее сообщение занимает 90 байт (может меньше из-за того, что некоторые символы, например пробелы будут
 занимать объем в 1 байт). Именно на таких тестовых сообщениях и будет тестироваться база.
